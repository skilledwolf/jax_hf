...
def test_variational_solver_with_project_fn_converges():
    ...
    P_fin, F_fin, E_fin, _mu_fin, k_fin, _history = runner(
        P0,
...
def test_variational_qr_solver_with_project_fn_converges():
    ...
    sigma_x = np.array([[0, 1], [1, 0]], dtype=np.complex64)
    G = jnp.stack([jnp.eye(2, dtype=jnp.complex64), jnp.asarray(sigma_x)], axis=0)
    proj = make_project_fn(unitary_group=G)
    ...
    sx = jnp.asarray(sigma_x)
    sxH = jnp.conj(sx.T)
    np.testing.assert_allclose(
        np.array(P_fin),
        np.array((sx @ P_fin) @ sxH),
        atol=1e-6,
        rtol=1e-6,
    )
    np.testing.assert_allclose(
        np.array(F_fin),
        np.array((sx @ F_fin) @ sxH),
        atol=1e-6,
        rtol=1e-6,
    )
