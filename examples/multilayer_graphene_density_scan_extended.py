#!/usr/bin/env python
"""Bilayer graphene density scan (extended): PM, SVP, SP, SVP_flipped.

Extends the baseline density scan to include two additional branches:

  * SP: spin-polarized but valley-symmetric.  Seed breaks spin symmetry;
    projection averages over {I, v3}.
  * SVP_flipped: same SVP projection (outlier_sv=(+1,+1)) but seed sign
    flipped.  Tests whether the SVP subspace is connected: if yes, should
    converge to the same state as SVP; if not, to a distinct configuration.
...
DEFAULT_OUTPUT = REPO_ROOT / "examples" / "outputs" / "direct_minimization_density_scan_extended.csv"
# Extended branch list includes SP and SVP_flipped (defined in _bilayer_common).
...
def main() -> int:
    ...
    if not args.no_figure:
        common.write_figure(
            ...
            title="Bilayer graphene PM/SVP/SP/SVP_flipped (direct minimization)",
